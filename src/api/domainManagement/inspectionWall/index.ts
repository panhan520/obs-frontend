...
export const getListApi = async (params: ICommonObj): Promise<ICommonGetListRes<IListItem[]>> => {
  try {
    ...
    const res: ICommonObj = await request.get('/inspect/wall/', { params })
...
export const editApi = (params: ICommonObj) => {
  return request.put(`/inspect/wall/${params.id}/`, params)
}
...
export const deleteApi = (params: ICommonObj) => {
  return request.delete(`/inspect/wall/${params.id}/`)
}

/** 获取详情 */
export const getDetailApi = async (params: ICommonObj) => {
  const res = await request.get(`/inspect/wall/${params.id}/`)
  return res.data
}
...
export const getRecordApi = async (params: ICommonObj) => {
  try {
    ...
    delete params.total
    delete params.pagination
    const res = await request.get('/inspect/wall-record/', { params })
    return Promise.resolve({
      list: res.data.data,
      pagination: res.data.pagination,
    })
